...
- **Obrigatorio**: caminho da pasta da aplicacao (ex: `.` para a raiz do projeto)
- **Opcional**: porta (se nao informada, detecta automaticamente ou usa a padrao da linguagem)
- **Opcional**: health check path (default: `/health` — endpoint ja existente neste projeto)

**Exemplos de invocacao neste projeto:**
```
/dockerfile-generator .
/dockerfile-generator . 8000 /health
```
...
**Nota para este projeto:** a raiz contem `requirements.txt` — sera detectado como Python/FastAPI. O entrypoint correto e `uvicorn src.api.main:app --host 0.0.0.0 --port 8000`.
...
2. **Pin de versao**: use tags especificas nas base images (ex: `python:3.11-slim`, nao `python:latest`)
3. **Alpine ou distroless**: prefira imagens alpine para runtime
...
8. **Sem segredos na imagem**: nunca copie .env, credentials, chaves privadas ou artefatos em models/
...
**Nota**: Para FastAPI use `uvicorn`; para Flask/Django ajuste o CMD conforme o framework detectado.
...
- O projeto base fica em `/home/samuelsales/DevOps-projetos/mlops-cloud-cost-anomaly-platform/`
- A aplicacao FastAPI esta em `src/api/main.py` — entrypoint: `uvicorn src.api.main:app`
- O endpoint `/health` ja existe e responde 200 — use como health check path padrao
- O Dockerfile para este projeto sera criado na raiz (Phase 4 — Docker)
- Nunca copie `models/` para dentro da imagem — os artefatos ML sao montados via volume ou baixados no startup
